<?php /** * Template Name: プロテイン *//** * Template Post Type: protein */  get_header(); ?>
<article>
  <section>
    <div class="container"> 
...
<?php endwhile; ?>
      <div class="gym-tag-title">
        <div class="gym-tag gym-tag_border">
          <ul><?php $terms = get_the_terms($post->ID, 'protein_tag'); if ($terms && !is_wp_error($terms)) { foreach ($terms as $term) { $term_link = get_term_link($term); if (!is_wp_error($term_link)) { echo '<a href="' . esc_url($term_link) . '"><li>' . esc_html($term->name) . '</li></a>'; } } } ?></ul>
        </div>
