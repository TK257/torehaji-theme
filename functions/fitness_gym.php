<?php
function fitness_gym_post_type()
{
  register_post_type(
    'fitness_gym',
    array(
...
add_action('init', 'fitness_gym_post_type', 1);
//カスタム投稿をカテゴリーページで一覧表示
function fitness_gym_main_query($query)
{
  if (is_admin() || !$query->is_main_query())
    return;

  if ($query->is_category() || $query->is_tag()) {
    $query->set('post_type', array('post', 'fitness_gym'));

    return;
  }
}
add_action('pre_get_posts', 'fitness_gym_main_query');
// カスタム投稿タイプを自作終了