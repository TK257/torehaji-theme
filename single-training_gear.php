...
<?php endwhile; ?>
      <div class="gym-tag-title">
        <div class="gym-tag gym-tag_border">
          <ul><?php $terms = get_the_terms($post->ID, 'gear_tag'); if ($terms && !is_wp_error($terms)) { foreach ($terms as $term) { $term_link = get_term_link($term); if (!is_wp_error($term_link)) { echo '<a href="' . esc_url($term_link) . '"><li>' . esc_html($term->name) . '</li></a>'; } } } ?></ul>
        </div>
