    ...
    <div class="container"> 
      <div class="main-waku">
        ...
        <div class="seacheria">
          ...
          <div class="none eriacity"><i class="fa-solid fa-magnifying-glass"></i>種類から探す
            <ul class="area"><?php  $terms = get_terms('protein_type'); foreach ( $terms as $term ) {  echo '<li>'; echo  '<a href="'.home_url().'/protein-type/'.$term->slug.'">'.$term->name.'</a></li>';}?></ul>
          </div><?php if(have_posts()): ?><?php while(have_posts()): the_post(); ?>
        </div>
        <div class="gymlist-waku">
          <div class="gymlist-gymkind">
            <ul><?php $terms = get_the_terms($post->ID, 'protein_type'); foreach($terms as $term) { echo '<li>'.$term->name.'</li>'; }?></ul>
          </div>
